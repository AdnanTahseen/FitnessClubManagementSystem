...
class YearlyPDF:
    ...
    def create_yearly_pdf(self):
        ...
        try:
            ...
            pdf.cell(200, 20, txt='Yearly Profit Table', ln=0, align='C')
            # creating list for holding table information
            yearly_profit_list=[('Profit ID','Profit Month','Profit Year','Total Expense', 'Total Income','Total Salaries','Total Profit')]
            # fetching monthly profit
            connection = pms.connect(host='localhost', port=3306, user='root', password='', database='fcms')
            cursor = connection.cursor()
            query = '''
                            SELECT * FROM profit where pro_year = %s
                            '''
            temp_profit_list=[]
            try:
                cursor.execute(query, (self.__report_year, ))
                for profit in cursor:
                    yearly_profit_list.append(profit)
                    temp_profit_list.append(profit[6])
                connection.close()

            except Exception as e:
                messagebox.showerror('Fitness Club Management System', 'Error: ' + str(e))
            # print(temp_profit_list)
            # calculating total yearly profit
            total_yearly_profit = 0
            for profit in temp_profit_list:
                total_yearly_profit = total_yearly_profit + profit

            # print(total_yearly_profit)
            # coverting the items 0f list of tuples into string one by one
            converted_list=[]
            for tup in yearly_profit_list:
                converted_items = tuple(str(item) for item in tup)
                converted_list.append(converted_items)

            yearly_profit_list.clear()
            yearly_profit_list.extend(converted_list)
            ...
            with pdf.table(text_align='CENTER', headings_style=headings_style) as table:
                for data_row in yearly_profit_list:
                    table_row = table.row()
                    for data_col in data_row:
                        table_row.cell(data_col)

            pdf.cell(0, 10, '', ln=1, align='C')
            pdf.set_font('arial', '', 12)
            pdf.set_text_color(255, 102, 0)
            pdf.cell(40, 10, txt='Total Yearly Profit: ', ln=0, align='C')
            # setting total yealy profit
            pdf.set_font('arial', '', 12)
            pdf.set_text_color(25, 0, 51)
            pdf.cell(10, 10, txt=str(total_yearly_profit), ln=1, align='C')
            ################################################################
            ...
            pdf_name='Reports/fcms_{day}_{month}_{year}_report(Yearly).pdf'.format(day=self.__day, month=self.__month, year=self.__year)
            pdf.output(pdf_name)
            return True
